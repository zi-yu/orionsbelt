...
namespace Chronos.Messaging.Messages {
	...
	/// </summary>
	[FactoryKey("BattleMove")]
	[Serializable]
	public sealed class BattleMove : NotReplyableMessageInfo {
	
		#region Ctors

		/// <summary>Construtor de Message</summary>
		public BattleMove() : base(MessageType.Battle) {
		}
		...
		public override string log( Message message )
		{
			...
			string src = args[1].ToString(  );
			string dst = args[1].ToString(  );

			return string.Format("{0} {1} where moved from coordinate {2} to coordinate {3}.",quant,type,src,dst);
		}
		...
		public override string localize( Message message, string localization, ITranslator translator )
		{
			...
			string quant = args[0].ToString(  );
			string type = args[1].ToString(  );
			string src = args[2].ToString(  );
			string dst = args[3].ToString(  );
