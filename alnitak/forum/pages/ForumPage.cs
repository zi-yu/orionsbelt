...
namespace yaf.pages
{
	...
	public class ForumPage : System.Web.UI.UserControl
	{
		...
		private void ForumPage_PreRender(object sender,EventArgs e)
		{
			...
			header.AppendFormat("<table width='100%' cellspacing='0' class='forumContent' cellpadding='0'><tr>");
		...
		protected override void Render(System.Web.UI.HtmlTextWriter writer) 
		{
			...
			footer.AppendFormat(GetText("COMMON","POWERED_BY"),
				String.Format("<a target=\"_top\" title=\"Yet Another Forum.net\" href=\"http://www.yetanotherforum.net/\">YAF</a>"),
				String.Format("{0}",Data.AppVersionName)
				);
			footer.AppendFormat("<br/>Copyright &copy; 2005 Orion's Belt Team");
...
#if !DEBUG
			doc = (XmlDocument)System.Web.HttpContext.Current.Cache[OrionGlobals.ForumSkinName];
#endif
			if(doc==null) {
				doc = new XmlDocument();
				//doc.Load(System.Web.HttpContext.Current.Server.MapPath(String.Format("{0}forum/{1}",Data.ForumRoot,themefile)));
				
				doc.Load(OrionGlobals.ResolveForumImagePath("forum.xml"));
#if !DEBUG
				System.Web.HttpContext.Current.Cache[OrionGlobals.ForumSkinName] = doc;
#endif
			}
			return doc;
		}
		...
		public string GetThemeContents(string page,string tag) 
		{
			...
			contents = contents.Replace("~",String.Format("{0}",OrionGlobals.getSkinImagePath( "forum/" + themeDir ) ) );
			return contents;
		}
	...
	/*	private string LoadTranslation() 
		{
			...
			if(filename.ToLower()!="english.xml") 
			{
				...
				{
					m_defaultLocale = new Localizer(HttpContext.Current.Server.MapPath(String.Format("{0}languages/en.xml",Data.ForumRoot)));
#if !DEBUG
		...
		private string LoadTranslation() {
			...
			string filename = CultureModule.RequestLanguage.Substring( 0,2 ) + ".xml";
			m_localizer = (Localizer)HttpContext.Current.Cache["Localizer." + filename ];
			if( null == m_localizer ) {
				m_localizer = new Localizer(HttpContext.Current.Server.MapPath(String.Format("{0}languages/{1}",Data.ForumRoot,filename)));
//#if !DEBUG
...
//#endif
		...
		public string ThemeFile(string filename) {
			return OrionGlobals.getSkinImagePath( "forum/" + filename );
		}
		#endregion
	}
}
