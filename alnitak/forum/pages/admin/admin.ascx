...
<yaf:adminmenu runat="server" id="Adminmenu1">
	<asp:repeater id="ActiveList" runat="server">
		<HeaderTemplate>
			<table width="100%" cellspacing="1" cellpadding="0" class="forumContent">
				<tr>
		...
		<ItemTemplate>
			<tr>
				...
				<td class="post"><%# DataBinder.Eval(Container.DataItem,"user_IP") %></td>
	...
	<asp:repeater id="UserList" runat="server">
		...
		<ItemTemplate>
			<tr>
				<td class="post"><%# DataBinder.Eval(Container.DataItem,"user_nick") %></td>
				<td class="post"><%# DataBinder.Eval(Container.DataItem,"user_mail") %></td>
				<td class="post"><%# DataBinder.Eval(Container.DataItem,"user_Location") %></td>
				<td class="post"><%# FormatDateTime(DataBinder.Eval(Container.DataItem,"user_registdate")) %></td>
				<td class="post">
					<asp:linkbutton onload="Approve_Load" runat="server" commandname="approve" commandargument='<%# DataBinder.Eval(Container.DataItem,"User_ID") %>' text="Approve"/>
					|
					<asp:linkbutton onload="Delete_Load" runat="server" commandname="delete" commandargument='<%# DataBinder.Eval(Container.DataItem,"User_ID") %>' text="Delete"/>
				</td>
			</tr>
		</ItemTemplate>
		<FooterTemplate>
