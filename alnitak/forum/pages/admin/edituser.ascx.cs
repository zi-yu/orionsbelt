		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack) {
				...
				using(DataTable dt = DB.user_list(PageBoardID,Request.QueryString["u"],null)) 
				{
					...
					Name.Text = (string)row["user_nick"];
					Email.Text = row["user_mail"].ToString();
					IsHostAdminX.Checked = ((int)row["user_Flags"] & (int)UserFlags.IsHostAdmin) == (int)UserFlags.IsHostAdmin;
					Joined.Text = row["user_registdate"].ToString();
					LastVisit.Text = row["user_lastlogin"].ToString();
					ListItem item = RankID.Items.FindByValue(row["user_RankID"].ToString());
					if(item!=null)
						item.Selected = true;
				}
			}
