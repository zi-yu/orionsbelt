...
namespace yaf.pages
{
	...
	public class forum : ForumPage
	{
		...
		protected HyperLink UnreadMsgs;
		protected HtmlGenericControl Welcome;
		protected Label activeinfo;
		...
		private void BindData() 
		{
			...
			if(!stats.IsNull("LastPost")) 
			{
				Stats.Text += String.Format(GetText("stats_lastpost"),
					...
					String.Format("<a href=\"{0}\">{1}</a>",string.Format("userinfo.aspx?id={0}",stats["LastUserID"]),Server.HtmlEncode(stats["LastUser"].ToString()))
				);
				Stats.Text += "<br/>";
			}
			...
			Stats.Text += String.Format(GetText("stats_lastmember"),
				String.Format("<a href=\"{0}\">{1}</a>",string.Format("userinfo.aspx?id={0}",stats["LastMemberID"]),Server.HtmlEncode(stats["LastMember"].ToString()))
				);
			Stats.Text += "<br/>";
		...
		protected string FormatLastPost(DataRow row) {
			if(!row.IsNull("LastPosted")) {
				...
				return String.Format("{0}<br/>{1}<br/>{2}&nbsp;<a title=\"{4}\" href=\"{5}\"><img src='{3}'></a>",
					...
					String.Format(GetText("by"),String.Format("<a href=\"{0}\">{1}</a>",string.Format("userinfo.aspx?id={0}",row["LastUserID"]),row["LastUser"])),
					minipost,
