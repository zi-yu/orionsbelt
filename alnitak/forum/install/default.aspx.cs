...
namespace yaf.install
{
	...
	public class _default : Page
	{
		...
		protected Button back, next, finish;
		protected Label cursteplabel;
		protected HtmlTable stepWelcome, stepConfig, stepConnect, stepDatabase, stepForum, stepFinished;
		protected PlaceHolder ConfigSample;
		// Forum
		protected TextBox TheForumName, UserName, Password1, Password2, AdminEmail, ForumEmailAddress, SmptServerAddress;
			...
			else if(CurStep == Step.Forum) 
			{
				...
				try 
				{
					using(SqlCommand cmd = new SqlCommand("yaf_system_initialize")) 
					{
						...
						cmd.Parameters.Add("@Password",FormsAuthentication.HashPasswordForStoringInConfigFile(Password1.Text,"md5"));
						DB.ExecuteNonQuery(cmd);
					}
