...
namespace FlexWiki
{
	...
	public class ContentBase : BELObject, IComparable
	{
		...
		ContentBase(string physicalPath, Federation aFederation)
		{
			...
			_Root = _Federation.FederationNamespaceMapFilename;
			int idx = _Root.LastIndexOf('/');
			_Root = _Root.Substring(0, idx+1) + physicalPath;
		...
		string DefinitionTopicFilePath
		{
			get
			{
				return Root + Path.DirectorySeparatorChar + DefinitionTopicFilename;
			}
		}
		...
		string TopicPath(TopicName localTopicName)
		{
			...
			if (cb == null) {
				throw new Exception("No ContentBase for Topic " + abs);
			}

			string str = cb.TopicPath(abs);
			return str;
		}
		...
		static string MakePath(string root, TopicName name)
		{
			if (name.Version == null || name.Version.Length == 0)
				return root + Path.DirectorySeparatorChar + name.Name + ".wiki";
			else
				return root + Path.DirectorySeparatorChar + name.Name + "(" + name.Version + ").awiki";
		}
		...
		public string ThisDirectory {
			get {
				return Path.Combine( 
						Path.GetDirectoryName(Federation.FederationNamespaceMapFilename),
						Namespace
					);
			}
		}
