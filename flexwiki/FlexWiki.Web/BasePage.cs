...
/*			string full = req.Url.ToString();
			...
			string host = ConfigurationSettings.AppSettings["WikiUrl"];
			if( host == null ) {
				throw new Exception("No 'WikiUrl' found in AppSettings");
			}
		...
		public void log( object obj, params object[] args )
		{
#if DEBUG
			System.Diagnostics.StackTrace trace = new System.Diagnostics.StackTrace(true);
		...
		{
			log("Estabelisinh Federation... ");
			if (TheFederation != null)
			{
				log("Federation Exists!");
				// If we have one, just make sure it's valid
				TheFederation.Validate();
				log("Federation Is Valid!");
				return;
			}

			// nope - need a new one
			log("Federation Map file: {0}", FederationNamespaceMapPath);
			...
			log("Federation Map file: {0}", fsPath);

			log("Federation - Default Namespace Dir: {0}", Federation.AbsoluteRoot("Orionsbelt", ""));
			log("Creating Federation...");
			TheFederation = new Federation(fsPath, OutputFormat.HTML, TheLinkMaker);
			log("Creating Federation... Done!");
			// Give the federation a cache to work with 
			//TheFederation.FederationCache = CacheManager;
			
			log("Federation - Default Namespace  : {0}", TheFederation.DefaultNamespace);
			log("Federation - Default Namespace  : {0}", TheFederation.DefaultContentBase.Namespace);
			foreach( ContentBase obj in TheFederation.ContentBases ) {
				log("Federation -                    : {0}", obj.Namespace);
			}
			log("Federation - Namespaces         : {0}", TheFederation.AllNamespaces.Count);
			foreach( object obj in TheFederation.AllNamespaces ) {
				log("Federation -                    : {0}", obj);
			}
			log("Federation - DefaultContentBase : {0}", TheFederation.DefaultContentBase == null ? "null": "ok");
			log("");
		}
